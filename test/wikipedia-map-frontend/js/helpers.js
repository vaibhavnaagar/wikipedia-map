...
// Get the color for the first node only
function getRedColor(level) {
  return lightenHex("#C80000 ",5*level); // Gets 5% lighter for each level
}
// Get the color that an edge should be pointing to a certain level
function getRedEdgeColor(level) {
  var nodecolor = getRedColor(level);
  return vis.util.parseColor(nodecolor).border;
}
...
function colorNodes(ns,color) {
  ...
  var colorFuncFirst = color ? getYellowColor : getRedColor;

  for (var i=0; i<ns.length; i++) {
    ns[i].color = ns[i].first ? colorFuncFirst(ns[i].level) : colorFunc(ns[i].level);
    // Prevent snapping
    delete ns[i].x;
    delete ns[i].y;
  }
