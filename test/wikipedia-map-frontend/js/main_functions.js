...
function expandNodeCallback(page,data) {
  ...
  var nodeColorFunc = getRedColor;
  var edgeColorFunc = getRedEdgeColor;
  var isFirst = true;
  ...
  for (var i=0; i<subpages.length; i++) {
    ...
    if (nodes.getIds().indexOf(subpageID) == -1) { //Don't add if node exists
        subnodes.push({id:subpageID, label:wordwrap(decodeURIComponent(subpage),15), value:1,
                       level:level, color:nodeColorFunc(level), parent:page,
                       x:nodeSpawn[0], y:nodeSpawn[1], first:isFirst}); //Add node
    }
    else {
      if (i == 0){
        var existingNode = nodes.get(subpageID);
        // existingNode.update({id:subpageID, first:true, color:nodeColorFunc()})
        existingNode.first = true;
        existingNode.color = nodeColorFunc(existingNode.level);
        nodes.update(existingNode);
      }
    }

    if (!getEdgeConnecting(page, subpageID)) { //Don't create duplicate edges in same direction
      newedges.push({from: page, to: subpageID, color:edgeColorFunc(level),
                     level: level, selectionWidth:2, hoverWidth:0, first:isFirst});
    }

    if (i == 0) {
      isFirst = false;
      nodeColorFunc = getColor;
      edgeColorFunc = getEdgeColor;
    }
  }
...
function resetProperties() {
  if (!isReset) {
    ...
    var modedges = traceedges.map(function(i){
      var e=edges.get(i);
      e.color = e.first ? getRedEdgeColor(nodes.get(e.to).level) : getEdgeColor(nodes.get(e.to).level);
      return e;
    });
